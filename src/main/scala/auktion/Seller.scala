...
class Seller(auctionTitles: List[String]) extends Actor {
  ...
  override def receive: Receive = {
    ...
    case Publish => {
      ...
      auctionTitles.map(title => registry ! Register(title, context.actorOf(Auction.props(FiniteDuration(10, TimeUnit.SECONDS), FiniteDuration(10, TimeUnit.SECONDS), sender, 10), "auction-" + title)))
    }
